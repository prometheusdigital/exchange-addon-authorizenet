...
function it_exchange_authorizenet_addon_process_webhook( $request ) {
	...
	if ( !empty( $request['x_trans_id'] ) && !empty( $request['x_MD5_Hash'] ) ) {
		...
		if ( $sent_md5 === $made_md5 ) {
			...
			error_log( sprintf( __( 'Unable to validate Silent Post from Authorize.Net. Please double check your MD5 Hash Value in the Authorize.Net settings in ExchangeWP and your Authorize.Net account: %s', 'it-l10n-ithemes-exchange' ), maybe_serialize( $request ) ) );
